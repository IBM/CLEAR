...
def _zip_looks_like_results(zf: zipfile.ZipFile) -> bool:
    """Heuristic: must have results*.csv or results*.parquet AND metadata*.json"""
    names = set(zf.namelist())
    has_results_csv = any(n.startswith("results") and n.endswith(".csv") for n in names)
    has_results_parquet = any(n.startswith("results") and n.endswith(".parquet") for n in names)
    has_metadata = any(n.startswith("metadata") and n.endswith(".json") for n in names)
    return (has_results_csv or has_results_parquet) and has_metadata


def collect_nested_results_zips(uploaded_file):
    """
    From a single uploaded zip, return a list of (display_name, BytesIO) for each *inner* zip
    that looks like a results package.

    If the top-level zip itself looks like a results zip, include it too so the user can choose it.
    """
    if uploaded_file is None:
        return []

    buf = uploaded_file.getvalue() if hasattr(uploaded_file, "getvalue") else uploaded_file.read()
    top_name = getattr(uploaded_file, "name", "uploaded.zip")

    candidates = []

    # Include top-level zip as a candidate if it is itself a valid results zip
    try:
        with zipfile.ZipFile(BytesIO(buf)) as top_zf:
            if _zip_looks_like_results(top_zf):
                bio = BytesIO(buf)
                bio.name = top_name.replace('.zip','')
                bio.seek(0)
                candidates.append((f"{top_name}", bio))

            # Scan nested zips (1 level deep)
            for member in top_zf.namelist():
                if not member.lower().endswith(".zip"):
                    continue
                try:
                    nested_bytes = top_zf.read(member)
                    nbio = BytesIO(nested_bytes)
                    nbio.name = f"{member.replace('.zip','')}"
                    nbio.seek(0)
                    with zipfile.ZipFile(nbio) as nested_zf:
                        if _zip_looks_like_results(nested_zf):
                            nbio.seek(0)
                            candidates.append((nbio.name, nbio))
    ...
    for name, bio in candidates:
        bio.seek(0)
        h = hashlib.md5(bio.getvalue()).hexdigest()
        if h not in seen:
            seen[h] = True
            deduped.append((name, bio))
    return deduped


def get_uploaded_file(uploader_key="zip_uploader"):
    # Single ZIP uploader (no folders, no multi-select)
    msg = "📁 Upload the ZIP (it can be a results zip OR a zip that contains multiple result zips)"
    uploaded_file = st.sidebar.file_uploader(msg, type="zip", key=uploader_key)

    if uploaded_file is not None:
        st.sidebar.info("Select which package to analyze from the dropdown below.")

        # Find candidates: top-level if valid + any nested result zips
        candidates = collect_nested_results_zips(uploaded_file)

        if not candidates:
            # Fallback: treat uploaded zip as raw (even if heuristic didn't match)
            # so previous single-zip flows still work for non-standard names.
            st.sidebar.warning(
                "Could not find nested result zips or standard results/metadata files. "
                "Attempting to read the uploaded file as a results package."
            )
            uploaded_file.seek(0)
            file_like = BytesIO(uploaded_file.getvalue())
            file_like.name = getattr(uploaded_file, "name", "uploaded.zip")
            file_id = file_hash(file_like)
            st.sidebar.success(f"Loaded file: {file_like.name}")
            return file_like, file_id

        names = [name for name, _ in candidates]

        # Preserve selection across reruns
        if "nested_zip_select_idx" not in st.session_state:
            st.session_state.nested_zip_select_idx = 0

        idx = st.sidebar.selectbox(
            "Choose a results ZIP to analyze:",
            options=list(range(len(names))),
            format_func=lambda i: names[i],
            index=st.session_state.nested_zip_select_idx,
            key="nested_zip_select_idx",
        )

        selected_name, selected_bio = candidates[idx]
        selected_bio.seek(0)
        file_id = file_hash(selected_bio)
        st.sidebar.success(f"Loaded file: {selected_name}")
        st.sidebar.info("❌ Remove the upload above to switch back to built‑in selection.")
        return selected_bio, file_id

    # --- No upload: keep your existing built-in results directory logic (unchanged) ---
    results_dir = os.getenv("CLEAR_EVAL_RESULTS_DIR")
    if results_dir:
        st.sidebar.markdown("Or select a built‑in configuration:")
        options = [d for d in os.listdir(results_dir) if os.path.isdir(os.path.join(results_dir, d))]
        if not options:
            st.warning("⚠️ No built-in configurations found in CLEAR_EVAL_RESULTS_DIR.")
            st.stop()

        selected_dataset_name = st.sidebar.radio("Choose a usecase to Analyze:", options=options,
                                                 key="use_case_selection")
        if selected_dataset_name:
            use_case_results_dir = os.path.join(results_dir, selected_dataset_name)
            data_options_all = os.listdir(use_case_results_dir)
            data_options = ["None"] + [
                d for d in data_options_all
                if os.path.isdir(os.path.join(use_case_results_dir, d)) and d != "final_results"
            ]
    ...
    st.warning("⚠️ Please upload a ZIP or use the built-in selection to proceed.")
    st.stop()
