...
def get_issues_mapping_system_prompt(shortcomings_list, format_mode="shortcomings"):
    ...
    if format_mode == "recommendations":
        ...
        matching_instruction = f"""A recommendation applies if the evaluation text indicates the model FAILED to follow that recommendation
...
- Use 1 if the evaluation clearly indicates the model **failed to follow** the recommendation (the problem it addresses is present)
- Use 0 if the recommendation was followed, or if there's no evidence of the problem it addresses
"""
        item_label = "Recommendation"
    else:  # shortcomings
        list_description = f"following {num_shortcomings} shortcomings (derived from common themes)"
        matching_instruction = f"""A shortcoming must be expressed or implied with **negative sentiment** to be marked.
