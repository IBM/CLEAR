...
3. Tool specs JSON files for each benchmark live in `dev/tool_specs/`
...
The converter (`dev/traces_to_clear_csv.py`) emits one row per LLM
...
```bash
python dev/traces_to_clear_csv.py \
    /path/to/traces/*.json \
    --output-dir dev/runs/<bench>/input \
    --specs-dir dev/tool_specs
```

## 2. Configure the run

[`dev/sparc_config.yaml`](sparc_config.yaml) holds the defaults. The
...
`dev/run_sparc.sh` wraps the CLI so you can mix `--track` × `--mode`:

```bash
# Default: slow_track, runtime mode, all 3 benches
bash dev/run_sparc.sh

# Single bench, eval mode (emits actionable recommendations)
bash dev/run_sparc.sh --track slow_track --mode eval appworld

# Fast track, multiple benches
bash dev/run_sparc.sh --track fast_track tau2_retail appworld
```

Output lands at `dev/runs/<bench>/output_<track>_<mode>/`.
