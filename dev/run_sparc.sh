...
#
# Per-benchmark input CSVs live under dev/runs/input/<bench>/<agent_type>
# Results go to
# "dev/runs/output/$bench/$agent_type/${track}_${mode}"
...
provider=$(grep "^provider:" dev/sparc_config.yaml | awk '{print $2}' | tr -d '"' | tr -d "'")
if [ -z "$provider" ]; then
  echo "error: provider not found in dev/sparc_config.yaml" >&2
  exit 1
fi
...
src = pathlib.Path("dev/sparc_config.yaml").read_text()
...
if [ ${#benches[@]} -eq 0 ]; then
  for bench_dir in dev/runs/input/*/; do
    [ -d "$bench_dir" ] || continue
    benches+=("$(basename "$bench_dir")")
  done
  if [ ${#benches[@]} -eq 0 ]; then
    echo "error: no benchmarks found in dev/runs/input/" >&2
    exit 1
  fi
fi
...
for bench in "${benches[@]}"; do
  ...
  if [ ${#agent_types[@]} -eq 0 ]; then
    ...
    for agent_dir in dev/runs/input/"$bench"/*/; do
  ...
  for agent_type in "${current_agent_types[@]}"; do
    for r in bad good; do
        data_dir="dev/runs/input/$bench/$agent_type/$r"
        results_dir="dev/runs/output/$bench/$agent_type/${track}_${mode}/$r"
